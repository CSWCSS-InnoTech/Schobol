...
namespace vavi.sound.pcm.resampling.ssrc
{
    using Java.IO;
    using Java.Nio;
    using static Java.Nio.ByteBuffer;
    using static Java.Nio.ByteOrder;
    using static Java.Nio.Channels.FileChannel;
    using Java.Util;
    using static Java.Util.Random;

    using vavi.util.I0Bessel;
    ...
    public class SSRC
{
    ...
    /** */
    //  private static const int M = 15;

    /** */
    private const int RANDBUFLEN = 65536;
    ...
    /** */
    private static readonly int[] scoeffreq = {
            0, 48000, 44100, 37800, 32000, 22050, 48000, 44100
    };

    /** */
    private static readonly int[] scoeflen = {
            1, 16, 20, 16, 16, 15, 16, 15
    };

    /** */
    private static readonly int[] samp = {
            8, 18, 27, 8, 8, 8, 10, 9
    };

    /** */
    private static readonly double[][] shapercoefs = {
            new double[] {-1}, // triangular dither

            new double[] {-2.8720729351043701172, 5.0413231849670410156, -6.2442994117736816406, 5.8483986854553222656,
            ...
            new double[] {-2.6773197650909423828, 4.8308925628662109375, -6.570110321044921875, 7.4572014808654785156,
            ...
            new double[] {-1.6335992813110351562, 2.2615492343902587891, -2.4077029228210449219, 2.6341717243194580078,
            ...
            new double[] {-0.82901298999786376953, 0.98922657966613769531, -0.59825712442398071289, 1.0028809309005737305,
            ...
            new double[] {-0.065229974687099456787, 0.54981261491775512695, 0.40278548002243041992, 0.31783768534660339355,
            ...
            new double[] {-2.3925774097442626953, 3.4350297451019287109, -3.1853709220886230469, 1.8117271661758422852,
            ...
            new double[] {-2.0833916664123535156, 3.0418450832366943359, -3.2047898769378662109, 2.7571926116943359375,
    ...
    public int init_shaper(int freq, int nch, int min, int max, int dtype, int pdf, double noiseamp)
    {
        ...
        {
            Android.Util.Log.Warn("InnoTecheLearning.AudioResampler",
                $"Warning: ATH based noise shaping for destination frequency {freq}Hz is not available, using triangular dither\n");
        }
        if (dtype == 2 || i == 6)
        {
            i = 0;
        ...
        switch (pdf)
        {
            case 0: // rectangular
                for (i = 0; i < RANDBUFLEN; i++)
                {
                    ...
                    randbuf[i] = noiseamp * (((double)r) / int.MaxValue - 0.5);
                }
                break;
            ...
            case 1: // triangular
                for (i = 0; i < RANDBUFLEN; i++)
                {
                    ...
                    r2 = pool[p];
                    pool[p] = random.NextInt();
                    randbuf[i] = noiseamp * ((((double)r1) / int.MaxValue) - (((double)r2) / int.MaxValue));
                }
                break;
    ...
    private void usage()
    {
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","http://shibatch.sourceforge.net/\n\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","usage: ssrc [<options>] <source wav file> <destination wav file>\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","options : --rate <sampling rate>     output sample rate\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --att <attenuation(dB)>    attenuate signal\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --bits <number of bits>    output quantization bit length\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --tmpfile <file name>      specify temporal file\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --twopass                  two pass processing to avoid clipping\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --normalize                normalize the wave file\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --quiet                    nothing displayed except error\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --dither [<type>]          dithering\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       0 : no dither\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       1 : no noise shaping\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       2 : triangular spectral shape\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       3 : ATH based noise shaping\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       4 : less dither amplitude than type 3\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --pdf <type> [<amp>]       select p.d.f. of noise\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       0 : rectangular\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       1 : triangular\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       2 : Gaussian\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","          --profile <type>           specify profile\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       standard : the default quality\n");
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","                                       fast     : fast, not so bad quality\n");
    }
    ...
    private void showprogress(double p)
    {
        ...
        t = JavaSystem.CurrentTimeMillis() - starttime;
        if (p == 0)
        {
            eta = 0;
        }
        ...
        {
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler"," %3d%% processed", pc);
            lastshowed2 = pc;
        }
        if (t != lastshowed)
        {
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler",", ETA =%4dmsec", eta);
            lastshowed = t;
        }
        Android.Util.Log.Error("InnoTecheLearning.AudioResampler","\r");
    }
    ...
    public double upsample(InputStream fpi, OutputStream fpo, int nch, int bps, int dbps, int sfrq, 
        int dfrq, double gain, int chanklen, bool twopass, int dither)
    {
...
// Android.Util.Log.Error("InnoTecheLearning.AudioResampler","iza = %g\n",iza);
...
//Android.Util.Log.Error("InnoTecheLearning.AudioResampler","%d : %g %g %g %g %g %g\n",i,stage2[i*2],stage2[i*2+1],buf2[ch,i*2],buf2[ch,i*2+1],re,im);
...
iza = I0Bessel.value(alp);
//Android.Util.Log.Error("InnoTecheLearning.AudioResampler","iza %f, alp: %f\n", iza, alp); // OK
...
//Android.Util.Log.Error("InnoTecheLearning.AudioResampler","1: %06d: %e\n", i + n1 / 2, stage1[i + n1 / 2]); // OK
            }
...
//}
//for (i = 0; i < ipsize; i++) {
// Android.Util.Log.Error("InnoTecheLearning.AudioResampler","ip: %06d: %d\n", i, fft_ip[i]); // OK
//}
//for (i = 0; i < wsize; i++) {
// Android.Util.Log.Error("InnoTecheLearning.AudioResampler","w: %06d: %e\n", i, fft_w[i]); // OK
//}
        }
...
//Android.Util.Log.Error("InnoTecheLearning.AudioResampler","O: %06d: %f\n", op + p * nch + ch, tmp);
                    }
    ...
    public static void main(String[] args) throws Exception
{
    ...
    /** */
    private static const double presets[] = {
            0.7, 0.9, 0.18
    };
...
//Android.Util.Log.Error("InnoTecheLearning.AudioResampler","dfrq: %d\n", dfrq);
                continue;
            }
        ...
        if (!quiet) {
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler","Shibatch sampling rate converter version " + VERSION + "(high precision/nio)\n\n");
        }
...
int c3 = bb.Get();
length = bb.getInt();
                Android.Util.Log.Error("InnoTecheLearning.AudioResampler","chunk: %c%c%c%c\n", c0, c1, c2, c3);
        ...
        if (!quiet) {
            ...
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler","frequency : %d -> %d\n", sfrq, dfrq);
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler","attenuation : %gdB\n", att);
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler","bits per sample : %d -> %d\n", bps* 8, dbps* 8);
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler","nchannels : %d\n", nch);
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler","length : %d bytes, %g secs\n", length, (double) length / bps / nch / sfrq);
            if (dither == 0) {
                Android.Util.Log.Error("InnoTecheLearning.AudioResampler","dither type : none\n");
            } else {
                Android.Util.Log.Error("InnoTecheLearning.AudioResampler","dither type : %s, %s p.d.f, amp = %g\n", dtype[dither], ptype[pdf], noiseamp);
            }
            Android.Util.Log.Error("InnoTecheLearning.AudioResampler","\n");
        }
...
int fptlen, sumread;
            ...
            if (!quiet) {
                Android.Util.Log.Error("InnoTecheLearning.AudioResampler","Pass 1\n");
            }
...
//Android.Util.Log.Error("InnoTecheLearning.AudioResampler","nch: %d, bps: %d, size: %d, sfrq: %d, dfrq: %d, ???: %d, ???: %d, twopass: %b, dither: %d\n", nch, bps, 8, sfrq, dfrq, 1, length / bps / nch, twopass, dither);
            if (normalize) {
                if (sfrq<dfrq) {
...
leis.clear();
                ...
                f = leis.getDouble();
//if (sumread < 100) {
// Android.Util.Log.Error("InnoTecheLearning.AudioResampler","2: %06d: %f\n", sumread, f);
//}
                f *= gain;
                sumread++;
...
//System.err.println("ft: " + ft);
                if (ft.delete() == false) {
                    Android.Util.Log.Error("InnoTecheLearning.AudioResampler","Failed to remove %s\n", ft);
                }
            }
        } else {
            if (sfrq<dfrq) {
        ...
        if (!twopass && peak[0] > 1) {
            if (!quiet) {
                Android.Util.Log.Error("InnoTecheLearning.AudioResampler","clipping detected : %gdB\n", 20 * Math.Log10(peak[0]));
            }
        }
