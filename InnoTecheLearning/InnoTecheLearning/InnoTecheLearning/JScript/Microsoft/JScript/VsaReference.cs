namespace Microsoft.JScript
{
    ...
    internal class VsaReference : VsaItem, IVsaReferenceItem, IVsaItem
    {
        ...
        internal VsaReference(VsaEngine engine, string itemName) : base(engine, itemName, VsaItemType.Reference, VsaItemFlag.None)
        {
        ...
        internal bool Compile(bool throwOnFileNotFound)
        {
            try
            {
                ...
                else if ((string.Compare(fileName, "microsoft.vsa.dll", StringComparison.OrdinalIgnoreCase) == 0) || (string.Compare(strA, "microsoft.vsa.dll", StringComparison.OrdinalIgnoreCase) == 0))
                {
                    this.assembly = base.engine.VsaModule.Assembly;
                }
            ...
            {
                throw new VsaException(VsaError.AssemblyExpected, this.assemblyName, exception);
            }
            catch (FileNotFoundException exception2)
            {
                if (throwOnFileNotFound)
                {
                    throw new VsaException(VsaError.AssemblyExpected, this.assemblyName, exception2);
                }
                return false;
            }
            catch (FileLoadException exception3)
            {
                throw new VsaException(VsaError.AssemblyExpected, this.assemblyName, exception3);
            }
            catch (ArgumentException exception4)
            {
                throw new VsaException(VsaError.AssemblyExpected, this.assemblyName, exception4);
            }
            catch (Exception exception5)
            {
                throw new VsaException(VsaError.InternalCompilerError, exception5.ToString(), exception5);
            }
            catch
            {
        ...
        private void Load()
        {
            ...
            {
                throw new VsaException(VsaError.InternalCompilerError, exception4.ToString(), exception4);
            }
            catch
            {
                throw new VsaException(VsaError.InternalCompilerError);
