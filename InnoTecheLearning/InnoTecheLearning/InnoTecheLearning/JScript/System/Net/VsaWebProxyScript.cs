    ...
    internal class VsaWebProxyScript : MarshalByRefObject, IWebProxyScript
    {
        ...
        private bool CompileScript(Uri engineScriptLocation, string scriptBody, Type helperType, out byte[] pe, out byte[] pdb)
        {
            ...
            try
            {
                ...
                IVsaReferenceItem item = this.engine.Items.CreateItem(name, VsaItemType.Reference, VsaItemFlag.None) as IVsaReferenceItem;
                ...
                IVsaCodeItem item2 = this.engine.Items.CreateItem("SourceText", VsaItemType.Code, VsaItemFlag.None) as IVsaCodeItem;
        ...
        public string Run(string url, string host) => 
            (CallMethod(this.scriptInstance, "ExecuteFindProxyForURL", new object[] { url, host }) as string);
