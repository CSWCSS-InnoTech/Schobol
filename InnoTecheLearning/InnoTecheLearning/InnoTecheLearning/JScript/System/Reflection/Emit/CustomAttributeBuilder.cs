namespace System.Reflection.Emit
{
    ...
    [ClassInterface(ClassInterfaceType.None), ComDefaultInterface(typeof(_CustomAttributeBuilder)), ComVisible(true), HostProtection(SecurityAction.LinkDemand, MayLeakOnAbort=true)]
    public class CustomAttributeBuilder : _CustomAttributeBuilder
    {
        ...
        private const byte SERIALIZATION_TYPE_SZARRAY = 0x1d;
        ...
        private void EmitValue(BinaryWriter writer, Type type, object value)
        {
            ...
            else
            {
                ...
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Argument_BadParameterTypeForCAB"), new object[] { str2 }));
            }
        }
        ...
        internal void InitCustomAttributeBuilder(ConstructorInfo con, object[] constructorArgs, PropertyInfo[] namedProperties, object[] propertyValues, FieldInfo[] namedFields, object[] fieldValues)
        {
            ...
            {
                parameterTypes = ((ConstructorBuilder) con).GetParameterTypes();
            }
            else
            {
                ParameterInfo[] parametersNoCopy = con.GetParametersNoCopy();
                parameterTypes = new Type[parametersNoCopy.Length];
                for (num = 0; num < parametersNoCopy.Length; num++)
                {
                    parameterTypes[num] = parametersNoCopy[num].ParameterType;
                }
            }
            ...
            for (num = 0; num < parameterTypes.Length; num++)
            {
                if (constructorArgs[num] != null)
                {
                    ...
                    {
                        throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, Environment.GetResourceString("Argument_BadParameterTypeForConstructor"), new object[] { num }));
                    }
                }
            }
        ...
        internal int PrepareCreateCustomAttributeToDisk(ModuleBuilder mod) => 
            mod.InternalGetConstructorToken(this.m_con, true).Token;
