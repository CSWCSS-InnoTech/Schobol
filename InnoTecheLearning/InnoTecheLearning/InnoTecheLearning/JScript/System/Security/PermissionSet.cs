namespace System.Security
{
    ...
    public class PermissionSet : ISecurityEncodable, ICollection, IEnumerable, IStackWalk, IDeserializationCallback
    {
        ...
        internal bool CanUnrestrictedOverride() => 
            this.m_canUnrestrictedOverride;
        ...
        internal bool CheckDemand(PermissionSet target, out IPermission firstPermThatFailed) => 
            this.IsSubsetOfHelper(target, IsSubsetOfType.CheckDemand, out firstPermThatFailed, true);
        ...
        internal bool CheckDeny(PermissionSet deniedSet, out IPermission firstPermThatFailed)
        {
            ...
            if (((deniedSet != null) && !deniedSet.FastIsEmpty()) && !this.FastIsEmpty())
            {
                ...
                while (internal2.MoveNext())
                {
                    ...
                    {
                        if (deniedSet.m_Unrestricted && current.CanUnrestrictedOverride())
                        {
                ...
                if (this.m_Unrestricted)
                {
                    ...
                    {
                        if ((internal3.Current is IPermission) && CodeAccessPermission.CanUnrestrictedOverride((IPermission) internal3.Current))
                        {
                            return false;
                        }
                    }
        ...
        internal bool CheckPermitOnly(PermissionSet target, out IPermission firstPermThatFailed) => 
            this.IsSubsetOfHelper(target, IsSubsetOfType.CheckPermitOnly, out firstPermThatFailed, true);
        ...
        public static byte[] ConvertPermissionSet(string inFormat, byte[] inData, string outFormat)
        {
            if (inData != null)
            {
                ...
                {
                    set.FromXml(new Parser(inData, Tokenizer.ByteTokenEncoding.ByteTokens).GetTopElement());
                }
                else if (inFormat.Equals("XMLUNICODE"))
                {
                    set.FromXml(new Parser(inData, Tokenizer.ByteTokenEncoding.UnicodeTokens).GetTopElement());
                }
                else if (inFormat.Equals("BINARY"))
                {
                    set.DecodeUsingSerialization(inData);
                }
                else
                {
                    return null;
                ...
                if (outFormat.Equals("XMLASCII") || outFormat.Equals("XML"))
                {
                    ...
                    writer.Write(set.ToXml().ToString());
                    writer.Flush();
                ...
                if (outFormat.Equals("XMLUNICODE"))
                {
                    ...
                    StreamWriter writer2 = new StreamWriter(stream2, Encoding.Unicode);
                    writer2.Write(set.ToXml().ToString());
                    writer2.Flush();
                    stream2.Position = 2L;
                    int num = ((int) stream2.Length) - 2;
                ...
                {
                    return set.EncodeUsingSerialization();
                }
            }
            return null;
        }
        ...
        private IPermission CreatePerm(object obj) => 
            CreatePerm(obj, this.m_ignoreTypeLoadFailures);
        ...
        {
            IPermission ip = this.CreatePerm(obj);
            if (ip == null)
            {
                return null;
            }
            if (this.m_Unrestricted && CodeAccessPermission.CanUnrestrictedOverride(ip))
            {
                ip = null;
            }
            this.CheckSet();
            this.m_permSet.SetItem(index, ip);
            if (ip != null)
            {
        ...
        private void DecodeAllPermissions()
        {
            ...
            else
            {
                ...
                {
                    IPermission ip = this.GetPermission(i);
                    if ((ip != null) && !CodeAccessPermission.CanUnrestrictedOverride(ip))
                    {
                        this.m_canUnrestrictedOverride = false;
                    }
                }
        ...
        private bool DecodeUsingSerialization(byte[] data)
        {
            ...
            if (obj2 != null)
            {
                set = (PermissionSet) obj2;
                this.m_Unrestricted = set.m_Unrestricted;
                ...
                if (IsPermissionTag(doc.GetTagForElement(num3), allowInternalOnly))
                {
                    try
                    {
                        ...
                        if ((token != null) && (obj2 != null))
                        {
                            ...
                            if ((this.m_Unrestricted && (obj2 is IPermission)) && CodeAccessPermission.CanUnrestrictedOverride((IPermission) obj2))
                            {
                                obj2 = null;
                            }
        ...
        public IEnumerator GetEnumerator() => 
            new PermissionSetEnumerator(this);
        ...
        internal bool IsSubsetOfHelper(PermissionSet target, IsSubsetOfType type, out IPermission firstPermThatFailed, bool ignoreNonCas)
        {
            ...
            if (this.m_permSet != null)
            {
                ...
                {
                    IPermission ip = this.GetPermission(i);
                    if ((ip != null) && !ip.IsSubsetOf(null))
                    {
                        IPermission permission = target.GetPermission(i);
                        if (!target.m_Unrestricted || !CodeAccessPermission.CanUnrestrictedOverride(ip))
                        {
                            CodeAccessPermission permission3 = ip as CodeAccessPermission;
                            if (permission3 == null)
                            {
                                if (!ignoreNonCas && !ip.IsSubsetOf(permission))
                                {
                                    firstPermThatFailed = ip;
                                    return false;
                                }
                                continue;
                            }
                            ...
                            switch (type)
                            {
                                ...
                                case IsSubsetOfType.CheckDemand:
                                    if (permission3.CheckDemand((CodeAccessPermission) permission))
                                    {
                                        break;
                                    }
                                    return false;
                                ...
                                case IsSubsetOfType.CheckPermitOnly:
                                    if (permission3.CheckPermitOnly((CodeAccessPermission) permission))
                                    {
                                        break;
                                    }
                                    return false;
                                ...
                                case IsSubsetOfType.CheckAssertion:
                                    if (permission3.CheckAssert((CodeAccessPermission) permission))
                                    {
                                        break;
                                    }
                                    return false;
        ...
        public virtual SecurityElement ToXml() => 
            this.ToXml("System.Security.PermissionSet");
        ...
        internal SecurityElement ToXml(string permName)
        {
            ...
            {
                IPermission current = (IPermission) internal2.Current;
                if (!this.m_Unrestricted || !CodeAccessPermission.CanUnrestrictedOverride(current))
                {
        ...
        public PermissionSet Union(PermissionSet other)
        {
            ...
            if (set.m_Unrestricted)
            {
                ...
                if (other.m_canUnrestrictedOverride)
                {
                    set.m_permSet = (this.m_permSet != null) ? new TokenBasedSet(this.m_permSet) : null;
                    return set;
                }
            }
            this.CheckSet();
