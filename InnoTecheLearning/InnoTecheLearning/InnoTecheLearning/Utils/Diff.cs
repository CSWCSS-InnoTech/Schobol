...
namespace InnoTecheLearning
{
    ...
    partial class Utils
    {
        ...
        public static class Diff
        {
            ...
            public struct Item
            {
                ...
                /// <summary>Number of deletions in Data A.</summary>
                public int DeletedA;
                /// <summary>Number of inserts in Data B.</summary>
    ...
    public static string TestHelper(Item []f) {
      ...
      return (ret.ToString());
            ...
            public static List<Item> DiffText(string TextA, string TextB)
            {
            ...
            /// <summary>
            /// Find the difference in 2 arrays of <typeparamref name="T"/>.
            /// </summary>
            /// <param name="ArrayA">A-version of the <typeparamref name="T"/>s (usualy the old one)</param>
            /// <param name="ArrayB">B-version of the <typeparamref name="T"/>s (usualy the new one)</param>
            /// <returns>Returns a array of Items that describe the differences.</returns>
            public static List<Item> DiffOf<T>(T[] ArrayA, T[] ArrayB) where T : IConvertible
            {
                int[] arrayA = new int[ArrayA.Length];
                int[] arrayB = new int[ArrayB.Length];
                for (int i = 0; i < ArrayA.Length; i++) arrayA[i] = Convert.ToInt32(ArrayA[i]);
                for (int i = 0; i < ArrayB.Length; i++) arrayB[i] = Convert.ToInt32(ArrayB[i]);
                // The A-Version of the data (original data) to be compared.
            ...
            private static int[] DiffCodes(string aText, Dictionary<string, int> h, bool trimSpace, bool ignoreSpace, bool ignoreCase)
            {
                ...
                for (int i = 0; i < Lines.Length; ++i)
                {
                    ...
                    if (h.TryGetValue(s, out int aCode))
                    {
                        Codes[i] = aCode;
                    }
                    else
            ...
            private static void LCS(DiffData DataA, int LowerA, int UpperA, DiffData DataB, int LowerB, int UpperB)
            {
                ...
                else
                {
                    ...
                    (int x, int y) smsrd = SMS(DataA, LowerA, UpperA, DataB, LowerB, UpperB);
