...
namespace InnoTecheLearning
{
    partial class Utils
    {
        ...
        public static string GetFileName(Sounds Sound)
        {
            switch (Sound)
            {
                ...
                case Sounds.Cello_A:
                    return "CelloA.wav";
                default:
                    throw new NotImplementedException($"Please add corrosponding conversion for {Sound.ToString()}.");
            }
        }
        public abstract class MusicStream : Stream
        {
            ...
            public Stream Content { get; protected set; } = null;
            public abstract TimeSpan Duration { get; set; }
            public abstract Encoding Format { get; }
            ...
            public abstract AudioTrackMode Mode { get; }
            ...
            /// Stream type.
            /// </summary>
            public StreamType Type { get; set; } = StreamType.Music;
        ...
        public class PcmWavStream : MusicStream
        {
#if false
            #region Mime Types
                public static Dictionary<string, string> MimeTypes = new Dictionary<string, string>
        {
            ...
            #endregion
                public PcmWavStream(Stream Content, string Extension = "wav", bool Loop = false)
                {
            ...
            #endregion
            public PcmWavStream(Stream File, bool Loop = false, float Volume = 1)
            {
                if (File is PcmWavStream) throw new InvalidOperationException($"Cannot wrap another {nameof(PcmWavStream)}.");
                Content = File; this.Loop = Loop; this.Volume = Volume;
            }
            public PcmWavStream(Sounds Sound, bool Loop = false, float Volume = 1)
                : this(Resources.GetStream("Sounds." + GetFileName(Sound)), Loop, Volume) { }

            //int? _SampleRate = null;
            /// <summary>
            /// Frequency in Hertz (Hz). 
            /// </summary>
            //False info => Always returns 0 (Unspecified). OPTIONAL. Do not enter unless you are sure about the value.
            ...
            /// Audio encoding.
            /// </summary>
            public override Encoding Format
            {
                get
                {
            ...
            /// </summary>.
            public override long SizeInBytes { get { return Content.Length; } }
            /// <summary>
            /// Mode. Stream or static. If content is more than 20 secs then use stream.
            /// </summary>
            public override AudioTrackMode Mode { get { return (AudioTrackMode)Convert.ToInt32(Duration > TimeSpan.FromSeconds(20)); } }
        ...
        public class ToneStream : MusicStream
        {
            ...
            { this.Frequency = Frequency; Duration = TimeSpan.FromSeconds(Seconds);
                base.Loop = Loop; base.Volume = Volume; Init(); }
            public ToneStream(int Frequency, TimeSpan Duration, bool Loop = false, float Volume = 1)
            ...
            private void Init()
            {
                unchecked
                {
                    ...
                    var Backing = new byte[2 * Samples];
                    ...
                    foreach (double dVal in sample)
                    {
                        ...
                        Backing[idx++] = (byte)(val & 0x00ff);
                        Backing[idx++] = (byte)((val & 0xff00) >> 8);
                    }
                    uint numsamples = (uint)Backing.Length;
                    ushort numchannels = (ushort)Channels;
                    ushort samplelength = 1; // in bytes
                    uint samplerate = (uint)SampleRate;

                    var f = new MemoryStream(44 + Backing.Length);
                    ...
                    wr.Write(Backing);
                    Content = this.Backing = new PcmWavStream(f);
                }
            }

            PcmWavStream Backing;

            public override int BitsPerSample => Channels * 8;

            public override int Channels => 1;

            public override TimeSpan Duration { get; set; }

            public override Encoding Format => Encoding.Pcm16bit;

            public override AudioTrackMode Mode => AudioTrackMode.Static;

            public override int SampleRate => 8000;

            public override int Samples => SampleRate * (int)Duration.TotalSeconds;

            public override long SizeInBytes => Backing.SizeInBytes;
